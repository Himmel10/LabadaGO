...
export default function CustomerMessagesScreen() {
  ...
  // Get shops and riders from orders
          ...
          <View style={styles.chatHeader}>
            ...
            <View style={styles.chatHeaderInfo}>
              ...
              <View style={styles.roleBadge}>
                {other?.role === 'rider' ? <Truck size={10} color={Colors.rider} /> : <Store size={10} color={Colors.primary} />}
                <Text style={[styles.roleText, { color: other?.role === 'rider' ? Colors.rider : Colors.primary }]}>
                  {other?.role === 'rider' ? 'Rider' : 'Shop'}
                </Text>
              </View>
            </View>
          </View>
          ...
          {filteredContacts.map((contact) => (
            ...
            >
              <View style={[styles.contactAvatar, { backgroundColor: contact.role === 'rider' ? Colors.riderLight : Colors.primaryFaded }]}>
                {contact.role === 'rider' ? <Truck size={18} color={Colors.rider} /> : <Store size={18} color={Colors.primary} />}
              </View>
              <View style={styles.contactInfo}>
                <Text style={styles.contactName}>{contact.name}</Text>
                <Text style={styles.contactRole}>{contact.role === 'rider' ? 'Rider' : 'Shop'}</Text>
              </View>
            </TouchableOpacity>
          ))}
          {filteredContacts.length === 0 && (
            <View style={styles.emptyState}>
              <Store size={40} color={Colors.textTertiary} />
      ...
      <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false}>
          ...
          return (
            ...
            >
              <View style={[styles.convoAvatar, { backgroundColor: other?.role === 'rider' ? Colors.riderLight : Colors.primaryFaded }]}>
                {other?.role === 'rider' ? <Truck size={20} color={Colors.rider} /> : <Store size={20} color={Colors.primary} />}
              </View>
        ...
        {conversations.length === 0 && (
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyStateSubtext}>Start a conversation with a shop or rider</Text>
